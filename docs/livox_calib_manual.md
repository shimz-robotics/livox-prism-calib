...
`data/input_data/HAP_config.json` の `lidar_configs[].extrinsic_parameter` に書き込み、  
...
| `--master PATH`      |            | `data/input_data/HAP_config.json`（リポジトリ内） | マスター HAP_config.json（キャリブ結果の反映先）        |
...
- 現場設定の真実（マスター）は本リポジトリの `data/input_data/HAP_config.json` です。IP 構成の変更やキャリブ結果はまずマスターに反映し、ドライバへは本スクリプトで配備します。ドライバ側 config を直接編集しても、次回の配備で上書きされます
...
3. hap_ip_map.yaml で HAP 番号 → LiDAR IP を解決
       ↓
4. プレビュー表示 → 確認後、マスター（data/input_data/HAP_config.json）を更新
       ↓
...
2. 該当 IP の extrinsic_parameter のみをすべて 0 に設定
       ↓
3. プレビュー表示 → 確認後、マスター（data/input_data/HAP_config.json）を更新
       ↓
...
| `data/input_data/HAP_config.json`             | 現場設定マスター（Git 管理、キャリブ結果の反映先）             |
